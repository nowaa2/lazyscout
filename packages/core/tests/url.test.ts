...
import { LOCAL_QA_POLICY, PUBLIC_SAAS_POLICY, checkTargetUrl, moduleNameFromUrl, normalizeUrl } from '@lazyscout/core'
...
describe('checkTargetUrl', () => {
  ...
  it('บล็อก credentials ที่ฝังอยู่ใน URL', () => {
    expect(checkTargetUrl('https://user:password@example.com', LOCAL_QA_POLICY).ok).toBe(false)
  })
})
